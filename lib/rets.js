...
Object.defineProperties( RETS.prototype, {
  request: {
    ...
    value: function request( query, cb ) {
      ...
      var url = Instance.get('url.get_meta');

      // Normalize parametrs.
      query   = 'object' === typeof query ? query : { type: 'METADATA-CLASS' };
      ...
      request({
        ...
        headers: { "User-Agent": "NODE-RETS/1.0" }
      }, function response( error, res, body ) {

        if(res.statusCode === 401){
          console.error('Problem with query '+url+' '+JSON.stringify(query));
          console.error( '401 Unauthorized Access\r\nCheck your username and password' );
          return this;
        }
        ...
        try {
          ...
          if( data.RETS[ 'RETS-RESPONSE' ] ) {
            ...
            data.RETS[ 'RETS-RESPONSE' ][0].split( "\r\n" ).forEach( function line_parser( line, index ) {
              ...
              var value   =  (split.length==2)?split[1].replace(/^\s+|\s+$/g, '' ):'';
